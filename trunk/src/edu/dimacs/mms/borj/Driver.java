 ...
 *  that runs machine learning experiments using BOXER.
    ...
    data), describing a {@link boxer.Suite suite} (list of {@link
    boxer.Discrimination discriminations}, each one consisting of
...
java [-Dmodel=tg|eg|trivial] [-Dverbose=true | -Dverbosity={0...3}] [-Drunid=RUN_ID]  borj.Driver command:file [command:file ...]
    ...
    <li>read-suite: Read the description of a "{@link boxer.Suite
    ...
    boxer.FeatureDictionary feature dictionary} (which can be used to
    ...
    coefficient matrix (or matrices) for {@link boxer.PLRMLearner
...
public class Driver {
	...
	// If any labels arrive from separate label files, they will
	// be stored here until use
	...
	for( ; q!=null; q=cm.next() ) {
	    ...
	    if (q.is(CMD.READ)) {
		// too late to read the classifier now
		...
		suite.serializeLearnerComplex(q.f); // save the entire model
