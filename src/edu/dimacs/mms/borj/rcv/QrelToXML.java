...
public class QrelToXML {
	...
	Suite suite =  (suitein !=null) ? new Suite(new File(suitein)) :
	    buildHiFreqSuite(d, outdir);
	...
	// Prepare, for each QREL file, the list of all discrimination
	// to be generated based on that file
	HashMap<String, Vector<Discrimination>> qrelFile2disList = new
	    HashMap<String, Vector<Discrimination>>();
	...
	*/
	...
	String outqrel = ht.getOption("outlabels",
				      outdir + File.separator + setName+ "-qrel.xml");
	System.out.println("Writing data set " + setName + " with labels for " + v.size() +
...
BOXER_Small_Regions
	...
	*/
    ...
    static private Suite buildHiFreqSuite(String d, String outdir)
	throws java.io.IOException    {
	Suite suite = new Suite("RCV_Small_20081206");

	int ntop = 10;
	String qrelTopic = "rcv1-v2.topics.qrels";
	SiPair[] topTopics = topCat(null /*docids*/, d + File.separator + qrelTopic, ntop);
	...
	for(int i=0; i<topTopics.length; i++) {
	    ...
	    dis.setLeftoversClass(RcvToXML.OTHER); // added in ver 0.5.007
	...
	dis.setDefaultClass(RcvToXML.NONE); // added in ver 0.5.002
	dis.setLeftoversClass(RcvToXML.OTHER); // added in ver 0.5.007
	...
	String outsuite = ht.getOption("outsuite", 
				       outdir + File.separator +"rcv-small-cat-suite.xml");
    ...
    */
	...
	@param replaceName If this parameter is not null, classes absent from the current suite will be replaced with the class of this name. A class of this name must already exist in the suite,
     */
    static void convertQrel(String fname, Vector<Discrimination> disList,
			    Suite suite, HashMap<Integer,DataPoint> docs)
    //			    String disName, String replaceName, Suite suite)
	throws IOException
    {
	for(QrelIterator it = new QrelIterator(fname); it.hasNext(); ) {
	    QrelEntry q= it.next();
		...
		// add the found class (if it's listed in this discr), or the 
		// leftover class
